...
@Keep
open class SliceFragment :
  SettingsPreferenceFragment(), SliceFragmentCallback, SliceShard.Callbacks {
  private var mSliceShard: SliceShard? = null

  override fun onCreate(savedInstanceState: Bundle?) {
    val uriString = arguments!!.getString(SlicesConstants.TAG_TARGET_URI)
    val initialTitle = arguments!!.getCharSequence(SlicesConstants.TAG_SCREEN_TITLE, "")
  ...
  override fun setSubtitle(subtitle: CharSequence?) {
    val view = this.view
    val decorSubtitle =
      if (view == null) null else view.findViewById<View>(R.id.decor_subtitle) as TextView
  ...
  override fun onCreateView(
    ...
    val newTitleContainer = themedInflater.inflate(R.layout.slice_title_container, container, false)
    ...
    val newContainer = themedInflater.inflate(R.layout.tv_settings_progress_bar, container, false)
  ...
  override fun onCreateRecyclerView(
    ...
    if (context != null) {
      recyclerView.addItemDecoration(FirstCategoryNoPaddingDecoration(requireContext()))
    }
    return recyclerView
  }

  private class FirstCategoryNoPaddingDecoration(private val context: Context) :
    ...
    override fun getItemOffsets(
      ...
      if (adapter !is PreferenceGroupAdapter) return

      if (position < adapter.itemCount) {
        val preference: Preference? = adapter.getItem(position)

        if (position > 0 && preference is PreferenceCategory) {
          val preferenceCategoryTopOffset =
            context.resources.getDimensionPixelSize(
              R.dimen.slice_preference_category_top_decoration
            )
