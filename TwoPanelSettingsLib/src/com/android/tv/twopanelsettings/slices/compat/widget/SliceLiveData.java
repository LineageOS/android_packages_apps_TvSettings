...
import com.android.tv.twopanelsettings.slices.compat.SliceMetadata;
...
import com.android.tv.twopanelsettings.slices.compat.SliceStructure;
...
public final class SliceLiveData {
  ...
  public static final Set<SliceSpec> SUPPORTED_SPECS =
      new ArraySet<>(
          Arrays.asList(
              SliceSpecs.BASIC, SliceSpecs.LIST, SliceSpecs.LIST_V2, OLD_BASIC, OLD_LIST));

  /**
   * Produces a {@link LiveData} that tracks a Slice for a given Uri. To use this method your app
   * must have the permission to the slice Uri.
   */
  ...
  /**
   * Produces a {@link LiveData} that tracks a Slice for a given Intent. To use this method your app
   * must have the permission to the slice Uri.
   */
  public static @NonNull LiveData<Slice> fromIntent(
   ...
   */
  public static @NonNull LiveData<Slice> fromIntent(
      @NonNull Context context, @NonNull Intent intent, @Nullable OnErrorListener listener) {
    return new SliceLiveDataImpl(context.getApplicationContext(), intent, listener);
  }

  /**
   * Produces a {@link LiveData} that tracks a Slice for a given InputStream. To use this method
   * your app must have the permission to the slice Uri.
   *
   * <p>This will not ask the hosting app for a slice immediately, instead it will display the slice
   * passed in through the input. When the user interacts with the slice, then the app will be
   * started to obtain the current slice and trigger the user action.
   */
  public static @NonNull LiveData<Slice> fromStream(
      @NonNull Context context, @NonNull InputStream input, OnErrorListener listener) {
    return fromStream(context, SliceViewManager.getInstance(context), input, listener);
  }

  /**
   * Same as {@link #fromStream(Context, InputStream, OnErrorListener)} except returns as type
   * {@link CachedSliceLiveData}.
   */
  public static @NonNull CachedSliceLiveData fromCachedSlice(
      @NonNull Context context, @NonNull InputStream input, OnErrorListener listener) {
    return fromStream(context, SliceViewManager.getInstance(context), input, listener);
  }
  ...
  @NonNull
  public static CachedSliceLiveData fromStream(
      @NonNull Context context,
      SliceViewManager manager,
  ...
  /**
   * Implementation of {@link LiveData}<Slice> that provides controls over how cached vs live slices
   * work.
   */
  public static class CachedSliceLiveData extends LiveData<Slice> {
    ...
    CachedSliceLiveData(
        ...
        final SliceViewManager manager,
    ...
    /**
     * Generally the InputStream are parsed asynchronously once the LiveData goes into the active
     * state. When this is called, regardless of state, the slice will be read from the input stream
     * and then the input stream's reference will be released when finished.
     *
     * <p>Calling parseStream() multiple times or after the stream has already been parsed
     * asynchronously will have no effect.
     */
    ...
    /**
     * Moves this CachedSliceLiveData into a "live" state, causing the providing app to start up and
     * provide an up to date version of the slice. After calling this method the slice will always
     * be pinned as long as this LiveData is in the active state.
     *
     * <p>If the slice has already received a click or goLive() has already been called, then this
     * method will have no effect.
     *
     * <p>Once goLive() has been called, there is no way to reverse it, this LiveData will then
     * behave the same way as one created using {@link #fromUri(Context, Uri)}.
     */
    public void goLive() {
