...
public class ListContent extends SliceContent {
  ...
  private void populate(Slice slice) {
    ...
    for (int i = 0; i < children.size(); i++) {
      ...
      boolean isNonRowContent =
          child.hasAnyHints(
              HINT_ACTIONS, HINT_SEE_MORE, HINT_KEYWORDS, HINT_TTL, HINT_LAST_UPDATED);
   ...
   */
  public DisplayedListItems getRowItems(
      int availableHeight, SliceStyle style, SliceViewPolicy policy) {
    if (policy.getMode() == MODE_SMALL) {
      return new DisplayedListItems(
          new ArrayList<>(Arrays.asList(getHeader())), /* hiddenItemCount= */ mRowItems.size() - 1);
    } else if (!policy.isScrollable() && availableHeight > 0) {
      return style.getListItemsForNonScrollingList(this, availableHeight, policy);
    }
    return new DisplayedListItems(style.getListItemsToDisplay(this), /* hiddenItemCount= */ 0);
  }
  ...
  public static int getListHeight(
      List<SliceContent> listItems, SliceStyle style, SliceViewPolicy policy) {
  ...
  private static SliceItem getSeeMoreItem(@NonNull Slice slice) {
    SliceItem item =
        SliceQuery.findTopLevelItem(slice, null, null, new String[] {HINT_SEE_MORE}, null);
    if (item != null) {
