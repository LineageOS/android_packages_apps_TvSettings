...
import static com.android.tv.twopanelsettings.slices.compat.core.SliceHints.ICON_IMAGE;
import static com.android.tv.twopanelsettings.slices.compat.core.SliceHints.RAW_IMAGE_LARGE;
import static com.android.tv.twopanelsettings.slices.compat.core.SliceHints.UNKNOWN_IMAGE;
import static com.android.tv.twopanelsettings.slices.compat.widget.SliceViewPolicy.MODE_LARGE;
import static com.android.tv.twopanelsettings.slices.compat.widget.SliceViewPolicy.MODE_SMALL;
...
public class SliceStyle {
  ...
  public SliceStyle(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    TypedArray a =
        context
            .getTheme()
            .obtainStyledAttributes(attrs, R.styleable.SliceView, defStyleAttr, defStyleRes);
    try {
      ...
      mHeaderTitleSize = (int) a.getDimension(R.styleable.SliceView_headerTitleSize, 0);
      mHeaderSubtitleSize = (int) a.getDimension(R.styleable.SliceView_headerSubtitleSize, 0);
      mVerticalHeaderTextPadding =
          (int) a.getDimension(R.styleable.SliceView_headerTextVerticalPadding, 0);

      mTitleSize = (int) a.getDimension(R.styleable.SliceView_titleSize, 0);
      mSubtitleSize = (int) a.getDimension(R.styleable.SliceView_subtitleSize, 0);
      mVerticalTextPadding = (int) a.getDimension(R.styleable.SliceView_textVerticalPadding, 0);

      mGridTitleSize = (int) a.getDimension(R.styleable.SliceView_gridTitleSize, 0);
      mGridSubtitleSize = (int) a.getDimension(R.styleable.SliceView_gridSubtitleSize, 0);
      int defaultVerticalGridPadding =
          context.getResources().getDimensionPixelSize(R.dimen.abc_slice_grid_text_inner_padding);
      mVerticalGridTextPadding =
          (int)
              a.getDimension(
      ...
      int defaultRowMinHeight =
          context.getResources().getDimensionPixelSize(R.dimen.abc_slice_row_min_height);
      mRowMinHeight = (int) a.getDimension(R.styleable.SliceView_rowMinHeight, defaultRowMinHeight);

      int defaultRowMaxHeight =
          context.getResources().getDimensionPixelSize(R.dimen.abc_slice_row_max_height);
      mRowMaxHeight = (int) a.getDimension(R.styleable.SliceView_rowMaxHeight, defaultRowMaxHeight);

      int defaultRowRangeHeight =
          context.getResources().getDimensionPixelSize(R.dimen.abc_slice_row_range_height);
      mRowRangeHeight =
          (int) a.getDimension(R.styleable.SliceView_rowRangeHeight, defaultRowRangeHeight);

      int defaultRowSingleTextWithRangeHeight =
          context
              .getResources()
              .getDimensionPixelSize(R.dimen.abc_slice_row_range_single_text_height);
      ...
      int defaultRowInlineRangeHeight =
          context.getResources().getDimensionPixelSize(R.dimen.abc_slice_row_range_inline_height);
      mRowInlineRangeHeight =
          (int)
              a.getDimension(
                  R.styleable.SliceView_rowInlineRangeHeight, defaultRowInlineRangeHeight);

      mExpandToAvailableHeight = a.getBoolean(R.styleable.SliceView_expandToAvailableHeight, false);
    ...
    mRowTextWithRangeHeight =
        r.getDimensionPixelSize(R.dimen.abc_slice_row_range_multi_text_height);
    mRowSelectionHeight = r.getDimensionPixelSize(R.dimen.abc_slice_row_selection_height);
    mRowTextWithSelectionHeight =
        r.getDimensionPixelSize(R.dimen.abc_slice_row_selection_multi_text_height);
    mRowSingleTextWithSelectionHeight =
        r.getDimensionPixelSize(R.dimen.abc_slice_row_selection_single_text_height);
    ...
    mGridRawImageTextHeight = r.getDimensionPixelSize(R.dimen.abc_slice_grid_raw_image_text_offset);
    ...
    mListLargeHeight = r.getDimensionPixelSize(R.dimen.abc_slice_large_height_internal);
  }
  ...
  public int getRowHeight(RowContent row, SliceViewPolicy policy) {
    ...
    if (row.getRange() == null && row.getSelection() == null && policy.getMode() != MODE_LARGE) {
      return maxHeight;
    }
    ...
    if (row.getSelection() != null) {
      ...
      int textAreaHeight =
          row.getLineCount() > 1 ? mRowTextWithSelectionHeight : mRowSingleTextWithSelectionHeight;
  ...
  public int getGridHeight(GridContent grid, SliceViewPolicy policy) {
    ...
    if (grid.isAllImages()) {
      ...
      boolean iconImagesOrNone =
          largestImageMode == ICON_IMAGE || largestImageMode == UNKNOWN_IMAGE;
      height =
          largestImageMode == RAW_IMAGE_LARGE
              ? (grid.getFirstImageSize(mContext).y + (twoLines ? 2 : 1) * mGridRawImageTextHeight)
              : (twoLines && !isSmall)
                  ? (hasImage ? mGridMaxHeight : mGridMinHeight)
                  : (iconImagesOrNone ? mGridMinHeight : mGridImageTextHeight);
    }
    int topPadding = grid.isAllImages() && grid.getRowIndex() == 0 ? mGridTopPadding : 0;
    int bottomPadding = grid.isAllImages() && grid.getIsLastIndex() ? mGridBottomPadding : 0;
  ...
  public int getListHeight(ListContent list, SliceViewPolicy policy) {
    ...
    int maxLargeHeight = maxHeight > 0 ? maxHeight : mListLargeHeight;
    ...
    int height =
        bigEnoughToScroll && !getExpandToAvailableHeight()
            ? maxLargeHeight
            : maxHeight <= 0 ? desiredHeight : Math.min(maxLargeHeight, desiredHeight);
    if (!scrollable) {
      height =
          getListItemsHeight(
              getListItemsForNonScrollingList(list, height, policy).getDisplayedItems(), policy);
    }
    return height;
  }
  ...
  /**
   * Returns a list of items that can fit in the provided height. If this list has a see more item
   * this will be displayed in the list if appropriate.
   *
   ...
   */
  @NonNull
  public DisplayedListItems getListItemsForNonScrollingList(
    ...
    if (list.getSeeMoreItem() != null
      ...
      while (visibleHeight > availableHeight && visibleItems.size() >= minItemCountForSeeMore) {
  ...
  private boolean shouldSkipFirstListItem(List<SliceContent> rowItems) {
    ...
    return getHideHeaderRow()
        && rowItems.size() > 1
        && rowItems.get(0) instanceof RowContent
