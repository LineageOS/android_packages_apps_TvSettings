...
public class SliceQuery {
  ...
  public static List<SliceItem> findAll(
      ...
      @Nullable final String[] hints,
      @Nullable final String[] nonHints) {
    ArrayList<SliceItem> ret = new ArrayList<>();
    findAll(
        toQueue(s),
        item ->
            checkFormat(item, format) && (hasHints(item, hints) && !hasAnyHints(item, nonHints)),
        ret);
    return ret;
  }
  ...
  /** */
  @Nullable
  public static SliceItem findSubtype(
      @Nullable Slice s, @Nullable final String format, @Nullable final String subtype) {
    if (s == null) {
      return null;
    }
    return findSliceItem(
        toQueue(s), item -> checkFormat(item, format) && checkSubtype(item, subtype));
  }

  /** */
  @Nullable
  public static SliceItem findSubtype(
      @Nullable SliceItem s, @Nullable final String format, @Nullable final String subtype) {
    if (s == null) {
      return null;
    }
    return findSliceItem(
  ...
  @Nullable
  private static SliceItem findSliceItem(
      @NonNull final Deque<SliceItem> items, @NonNull Filter<SliceItem> f) {
    while (!items.isEmpty()) {
      ...
      if (item != null
          && (FORMAT_SLICE.equals(item.getFormat()) || FORMAT_ACTION.equals(item.getFormat()))) {
  ...
  private static void findAll(
      @NonNull final Deque<SliceItem> items,
      @NonNull Filter<SliceItem> f,
      @NonNull List<SliceItem> out) {
    while (!items.isEmpty()) {
      SliceItem item = items.poll();
      if (f.filter(item)) {
        out.add(item);
  ...
  @Nullable
  public static SliceItem findTopLevelItem(
      @NonNull Slice s,
      @Nullable final String format,
  ...
  public static SliceItem findItem(@NonNull Slice s, @NonNull final Uri uri) {
    return findSliceItem(
        toQueue(s),
        input -> {
          if (FORMAT_ACTION.equals(input.getFormat()) || FORMAT_SLICE.equals(input.getFormat())) {
