...
public class SliceProviderWrapperContainer {
  ...
  public static class SliceProviderWrapper extends SliceProvider {
    ...
    public SliceProviderWrapper(
        com.android.tv.twopanelsettings.slices.compat.SliceProvider provider,
        String[] autoGrantPermissions) {
      super(autoGrantPermissions);
      mAutoGrantPermissions =
          autoGrantPermissions == null || autoGrantPermissions.length == 0
              ? null
    ...
    public PendingIntent onCreatePermissionRequest(Uri sliceUri) {
      ...
      PendingIntent action =
          mSliceProvider.onCreatePermissionRequest(sliceUri, getCallingPackage());
      if (action != null) {
        return action;
      }
    ...
    private void checkPermissions(Uri uri) {
      if (uri != null) {
        for (String pkg : mAutoGrantPermissions) {
          if (getContext().checkCallingPermission(pkg) == PackageManager.PERMISSION_GRANTED) {
    ...
    @Override
    public Parcelable onBindSlice(Uri sliceUri, Set<SliceSpec> supportedVersions, Bundle extras) {
      ...
      try {
        ...
        com.android.tv.twopanelsettings.slices.compat.SliceProvider.setSpecs(null);
      }
    }
