...
import static com.android.tv.twopanelsettings.slices.compat.core.SliceHints.ACTION_WITH_LABEL;
import static com.android.tv.twopanelsettings.slices.compat.core.SliceHints.ICON_IMAGE;
import static com.android.tv.twopanelsettings.slices.compat.core.SliceHints.LARGE_IMAGE;
import static com.android.tv.twopanelsettings.slices.compat.core.SliceHints.RAW_IMAGE_LARGE;
import static com.android.tv.twopanelsettings.slices.compat.core.SliceHints.RAW_IMAGE_SMALL;
import static com.android.tv.twopanelsettings.slices.compat.core.SliceHints.SMALL_IMAGE;
import static com.android.tv.twopanelsettings.slices.compat.core.SliceHints.SUBTYPE_ACTION_KEY;
import static com.android.tv.twopanelsettings.slices.compat.core.SliceHints.SUBTYPE_DATE_PICKER;
import static com.android.tv.twopanelsettings.slices.compat.core.SliceHints.SUBTYPE_TIME_PICKER;
...
public class SliceActionImpl implements SliceAction {
  ...
  @Nullable private Parcelable mAction;

  // Either mAction or mActionItem must be non-null.
  @Nullable private SliceItem mActionItem;
  ...
  @NonNull private ActionType mActionType = ActionType.DEFAULT;

  private boolean mIsChecked;
  private int mPriority = -1;
  private long mDateTimeMillis = -1;

  @Nullable private SliceItem mSliceItem;
   ...
   * the image, {@link SliceHints#ICON_IMAGE} will be presented as a tintable icon. Note that there
   * is no difference between {@link SliceHints#SMALL_IMAGE} and {@link SliceHints#LARGE_IMAGE} for
   * actions; these will just be represented as an non-tintable image.
   *
   * @param action the pending intent to invoke for this action.
   * @param actionIcon the icon to display for this action.
  ...
  public SliceActionImpl(
      @NonNull Parcelable action,
      @NonNull IconCompat actionIcon,
      @SliceHints.ImageMode int imageMode,
      @NonNull CharSequence actionTitle) {
    mAction = action;
    mIcon = actionIcon;
    mTitle = actionTitle;
  ...
  /**
   * Constructs a SliceAction based off of a {@link SliceItem}. Expects a specific format for the
   * item.
   *
  ...
  public SliceActionImpl(@NonNull SliceItem slice) {
    ...
    SliceItem titleItem =
        SliceQuery.find(actionItem.getSlice(), FORMAT_TEXT, HINT_TITLE, null /* nonHints */);
    if (titleItem != null) {
      mTitle = titleItem.getSanitizedText();
    }
    SliceItem cdItem =
        SliceQuery.findSubtype(actionItem.getSlice(), FORMAT_TEXT, SUBTYPE_CONTENT_DESCRIPTION);
    if (cdItem != null) {
    ...
    if (actionItem.getSubType() == null) {
      ...
      switch (actionItem.getSubType()) {
        ...
        case SUBTYPE_DATE_PICKER:
          ...
          SliceItem dateItem = SliceQuery.findSubtype(actionItem, FORMAT_LONG, SUBTYPE_MILLIS);
          if (dateItem != null) {
        ...
        case SUBTYPE_TIME_PICKER:
          ...
          SliceItem timeItem = SliceQuery.findSubtype(actionItem, FORMAT_LONG, SUBTYPE_MILLIS);
          if (timeItem != null) {
    ...
    SliceItem priority =
        SliceQuery.findSubtype(actionItem.getSlice(), FORMAT_INT, SUBTYPE_PRIORITY);
    mPriority = priority != null ? priority.getInt() : -1;
    SliceItem actionKeyItem =
        SliceQuery.findSubtype(actionItem.getSlice(), FORMAT_TEXT, SUBTYPE_ACTION_KEY);
  ...
  private Slice.Builder buildSliceContent(@NonNull Slice.Builder builder) {
    ...
    if (mIcon != null) {
      ...
      if (mImageMode == ACTION_WITH_LABEL) {
        ...
        hints = mImageMode == ICON_IMAGE ? new String[] {} : new String[] {HINT_NO_TINT};
      }
