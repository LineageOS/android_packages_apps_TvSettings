 ...
 * content changes due to a callback from user interaction, then {@link
 ...
 * </pre>
 *
 * <p>Slices can be identified by a Uri or by an Intent. To link an Intent with a slice, the
 * provider must have an {@link IntentFilter} matching the slice intent. When a slice is being
 * requested via an intent, {@link #onMapIntentToUri(Intent)} can be called and is expected to
 * return an appropriate Uri representing the slice.
 *
 * <pre class="prettyprint">
 * {@literal
 * <provider
...
public abstract class SliceProvider extends ContentProvider {
  /**
   * This is the Android platform's MIME type for a URI containing a slice implemented through
   * {@link SliceProvider}.
   */
   ...
   *
   * <p>When an app tries to bind a slice from this provider that it does not have access to, This
   ...
   *
   * <p>The slice returned should have a spec that is compatible with one of the supported specs.
   *
   ...
   *
   * <p>Any intent filter added to a slice provider should also contain {@link
   * SliceManager#CATEGORY_SLICE}, because otherwise it will not be detected by {@link
   ...
   * wants to show slices from this app. The default implementation launches a dialog that allows
   * the user to grant access to this slice. Apps that do not want to allow this user grant, can
   * override this and instead launch their own dialog with different behavior.
   *
  ...
  public Bundle call(String method, String arg, Bundle extras) {
    if (method.equals(METHOD_SLICE)) {
      ...
      Parcelable s =
          handleBindSlice(uri, supportedSpecs, callingPackage, callingUid, callingPid, extras);
      Bundle b = new Bundle();
      b.putParcelable(EXTRA_SLICE, s);
      return b;
    } else if (method.equals(METHOD_MAP_INTENT)) {
      Intent intent =
          BundleCompat.getParcelable(extras, EXTRA_INTENT, android.content.Intent.class);
      if (intent == null) {
        return null;
      }
  ...
  private Parcelable handleBindSlice(
      Uri sliceUri,
      List<SliceSpec> supportedSpecs,
      String callingPkg,
      int callingUid,
    ...
    try {
      mSliceManager.enforceSlicePermission(sliceUri, callingPid, callingUid, mAutoGrantPermissions);
  ...
  public Slice createPermissionSlice(Context context, Uri sliceUri, String callingPackage) {
    return new Slice.Builder(sliceUri, null).build();
  }

  public static PendingIntent createPermissionPendingIntent(
      Context context, Uri sliceUri, String callingPackage) {
  ...
  public static Intent createPermissionIntent(
      Context context, Uri sliceUri, String callingPackage) {
