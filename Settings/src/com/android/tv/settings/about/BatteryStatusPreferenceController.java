...
/**
 * Concrete subclass of battery status preference controller
 */
public class BatteryStatusPreferenceController extends AbstractPreferenceController
    ...
    public BatteryStatusPreferenceController(Context context, Lifecycle lifecycle) {
        super(context);
        mContext = context;
        if (lifecycle != null) {
    ...
    private Handler getHandler() {
        if (mHandler == null) {
            mHandler = new BatteryStatusHandler(this);
        }
        return mHandler;
    }
    ...
    private void updateBattery() {
        ...
        if (mBatteryStatus != null) {
            String batterystatus = mContext.getString(
            ...
            switch (intent.getIntExtra(BatteryManager.EXTRA_STATUS, -1)) {
                case BatteryManager.BATTERY_STATUS_CHARGING:
                    batterystatus = mContext.getString(
                ...
                case BatteryManager.BATTERY_STATUS_FULL:
                    batterystatus = mContext.getString(
                        com.android.settingslib.R.string.battery_info_status_full);
                    break;
    ...
    private static class BatteryStatusHandler extends Handler {
        private WeakReference<BatteryStatusPreferenceController> mStatus;

        BatteryStatusHandler(BatteryStatusPreferenceController activity) {
            mStatus = new WeakReference<>(activity);
        }

        @Override
        public void handleMessage(Message msg) {
            BatteryStatusPreferenceController status = mStatus.get();
            if (status == null) {
                return;
            }
