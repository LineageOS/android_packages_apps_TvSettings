...
public class AppManagementFragment extends SettingsPreferenceFragment {
  ...
  public void onActivityResult(int requestCode, int resultCode, Intent data) {
    ...
    switch (requestCode) {
      case REQUEST_UNINSTALL:
        final int deleteResult =
            data != null ? data.getIntExtra(Intent.EXTRA_INSTALL_RESULT, 0) : 0;
    ...
    switch (resultCode) {
      case RESULT_CLEAR_DATA:
        clearData();
        break;
      case RESULT_CLEAR_CACHE:
        clearCache();
        break;
    }
  }
      ...
      try {
        if (preference.equals(mUninstallPreference)) {
          startActivityForResult(
              intent,
              mUninstallPreference.canUninstall() ? REQUEST_UNINSTALL : REQUEST_UNINSTALL_UPDATES);
        } else if (preference.equals(mClearDataPreference)
            || preference.equals(mClearCachePreference)) {
        ...
        Toast.makeText(
                getContext(), R.string.device_apps_app_management_not_available, Toast.LENGTH_SHORT)
            .show();
      }
      return true;
    } else {
  ...
  private void updatePrefs() {
    ...
    permissionsPreference.setIntent(
        new Intent(Intent.ACTION_MANAGE_APP_PERMISSIONS)
